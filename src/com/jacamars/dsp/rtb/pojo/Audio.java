...
public class Audio {
	...
	public boolean matched(Creative c, BidRequest br, String adId,  StringBuilder errorString, Probe probe) throws Exception {
		...
		if (mimeTypes.contains(c.mime_type)==false) {
			if (errorString != null) {
		...
		if (protocols != null) {
			...
			if (protocols.contains(c.audio_protocol)==false) {
				if (errorString != null) {
		...
		if (startdelay != -1) {
			...
			if (c.audio_start_delay != null && c.audio_start_delay > startdelay) {
				if (errorString != null) {
