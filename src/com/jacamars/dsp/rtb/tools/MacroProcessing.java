...
public class MacroProcessing {
	...
	/*
	 * {redirect_url}", config.redirectUrl); {pixel_url}", config.pixelTrackingUrl);
	 * 
	 ...
	 * {lat}",lat); {lon}", lon); {site_domain}",br.siteDomain); {pub}", exchange);
	 * {bid_id}", oidStr); {ad_id}", adid);" + " replaceAll(sb, "%7Bpub%7D",
	 * exchange); replaceAll(sb, "%7Bbid_id%7D", oidStr); replaceAll(sb,
	 * "%7Bad_id%7D", adid); replaceAll(sb, "%7Bsite_id%7D", br.siteId);
	 ...
	 * @param br     BidRequest. The bid request to pull values out of for macro
	 *               substitution.
	 * @param creat  Creative. The creative to pull values out of for macro
	 *               substitution.
	 * @param imp    Impression. The impression being bid on, to pull values out of
	 *               for substitution.
	...
	public static void replace(List<String> list, BidRequest br, Creative creat, Impression imp, String adid,
			StringBuilder sb, StringBuilder snurl, String dealid) throws Exception {
	...
	public static void replace(List<String> list, StringBuilder sb) throws Exception {
	...
	/**
	 * If the key has "_enc" in it or, contains the encoded {%7B} preamble - encode
	 * it, otherwise just return the value.
	 * 
	...
	/**
	 * Find macros in a string, (the creative offered) and stick them in the macros
	 * list.
	 * 
	 * @param macros List. A list of strings, you provide this and call this method
	 *               for every creative and you end up with the list of all known
	 *               macros used. Psrt of the campaign compilation.
