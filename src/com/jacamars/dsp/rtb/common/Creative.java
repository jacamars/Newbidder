...
public class Creative implements Serializable {
	...
	public static Creative getInstance(ResultSet rs) throws Exception {
	...
	public static List<Creative> getInstances(ResultSet rs) throws Exception {
		ArrayNode inner = JdbcTools.convertToJson(rs);
	...
	static PreparedStatement doUpdate(Creative c, Connection conn) throws Exception {
		...
		String sql = "UPDATE " + table + " SET " + "interval_start=?," + "interval_end=?," + "total_budget=?,"
				...
				+ "cur=?," + "type=?," + "ext_spec=?," + "attr=?, campaign_id=?,";
	...
	static void updateRules(String table, int id, TokenData td) throws Exception {
		...
		while (rs.next()) {
			...
			Campaign.touchCampaignWithCreative(c.campaign_id);
		}
	...
	void encodeUrl() {
		...
		//System.out.println("["+type+":"+id+"] +++>"+forwardurl);
		//System.out.println("============>"+encodedFurl);
	...
	public void update(JsonNode myNode) throws Exception {
		...
		if (node.get("campaign_id") == null)
			campaign_id = 0;
		else
			campaign_id = node.get("campaign_id").asInt();
