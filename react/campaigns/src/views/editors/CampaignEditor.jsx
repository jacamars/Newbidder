...
const getAttachedCreatives = () => {
  ...
  for (var x of props.avail) {
    var list = mt[x.type];
    if (list != undef && list.indexOf(x.id) != -1)
      items.push(<option key={"creatives-"+x.id} selected value={x.name}>{x.type} {" - "}{x.name}</option>);
    else
      items.push(<option key={"creatives-"+x.id} value={x.name}>{x.type} {" - "}{x.name}</option>);
  }
  return(items);
}
...
const  addNewCampaign = async () => {
    ...
    var copy = JSON.parse(JSON.stringify(campaign));
...
const getSelectedRules = () => {
  ...
  for (var i=0;i<vx.rules.length;i++) {
    ...
    if (x.customer_id === campaign.customer_id) {
