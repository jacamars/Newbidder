...
def GetBudget(*arg):
    try:
        if len(arg)==0:
            r = requests.post(globalHost, data='{"type":"GetBudget#"'+',"token":"'+token+'"}')
        if len(arg)==1:
        ...
        if len(arg)==2:
            r = requests.post(globalHost, data='{"type":"GetBudget#","campaign":"' + arg[0] + 
                              '","creative":"' + arg[1] + '","token":"'+token+'"}')
        if len(arg)==3:
            r = requests.post(globalHost, data='{"type":"GetBudget#","campaign":"' + arg[0] + 
                              '","creative":"' + arg[1] +
                              '","adtype":"' + arg[2] + '","token":"'+token+'"}')
        print (r.status_code, r.reason)
        data = json.loads(r.text)
        print (json.dumps(data, indent=4, sort_keys=True))
    except requests.exceptions.RequestException as e:
...
def GetValues(*arg):
    try:
        if len(arg)==0:
            r = requests.post(globalHost, data='{"type":"GetValues#"'+',"token":"'+token+'"}')
        if len(arg)==1:
...
GetToken("rtb4free","ben.faul@rtb4free.com","zulu");
